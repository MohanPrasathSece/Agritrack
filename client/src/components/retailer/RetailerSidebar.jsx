...
import { LayoutDashboard, ShoppingCart, Package, Truck, CreditCard, User, Settings, Menu, X, LogOut } from "lucide-react";
...
const navItems = [
    ...
    { title: "IoT Delivery Transit", url: "/retailer/delivery", icon: Truck },
...
const SidebarContent = ({ navItems, location, setMobileOpen, handleLogout, user }) => (
    <div className="flex h-full flex-col bg-white text-slate-600 border-r border-slate-200">
        ...
        <div className="flex flex-col items-center justify-center px-4 py-5 border-b border-slate-100">
            ...
            <span className="text-[10px] font-black uppercase tracking-wider text-emerald-600 mt-1">Retailer Terminal</span>
        </div>
        ...
        <div className="p-3 border-t border-slate-100">
            ...
            <div className="flex items-center gap-2.5 px-3 py-2 bg-slate-50 rounded-xl mb-2">
                <div className="w-8 h-8 rounded-full bg-emerald-600 flex items-center justify-center text-white font-bold text-xs shadow-sm">
                    {user?.name?.charAt(0) || user?.email?.[0]?.toUpperCase() || "R"}
                </div>
                <div className="flex-1 min-w-0">
                    <p className="text-xs font-bold text-slate-900 truncate">{user?.name || user?.email?.split('@')[0] || "Retailer"}</p>
            ...
            <div className="lg:hidden">
                <div className="fixed left-4 top-3 z-50 flex items-center gap-2">
                    <button
                        ...
                        className="rounded-lg bg-white p-1.5 shadow-sm border border-slate-200"
                    >
