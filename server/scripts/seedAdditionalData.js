...
async function seedAdditionalData() {
  try {
    ...
    const newCrops = additionalCrops.filter(crop => 
      !existingCropNames.has(`${crop.name}-${crop.variety}`)
    );

    console.log(`🌾 Adding ${newCrops.length} new crop varieties`);
    ...
    const retailers = existingUsers.filter(user => user.role === 'retailer');
    const aggregators = existingUsers.filter(user => user.role === 'aggregator');

    if (retailers.length === 0) {
      console.log('⚠️ No retailers found, skipping order creation');
    } else {
      // Get all available crops including newly created ones
      const { data: allCrops } = await supabase.from('crops').select('*').eq('status', 'listed');
      ...
      if (allCrops && allCrops.length > 0) {
        for (let i = 0; i < Math.min(5, allCrops.length); i++) {
          ...
          const quantity = Math.min(crop.quantity, 30);
    ...
    // Create collections for aggregators
    if (aggregators.length === 0) {
      console.log('⚠️ No aggregators found, skipping collection creation');
    } else {
      console.log('🚚 Creating sample collections...');
      const { data: allCrops } = await supabase.from('crops').select('*').eq('status', 'listed');
      
      if (allCrops && allCrops.length > 0) {
        for (let i = 0; i < Math.min(3, allCrops.length); i++) {
          const crop = allCrops[i];
          const aggregator = aggregators[i % aggregators.length];
          const collectedQuantity = Math.min(crop.quantity, 80);
    ...
    // Update user profiles with more realistic data
    console.log('👤 Updating user profiles...');
    for (const user of existingUsers) {
      try {
        let updateData = {};
        
        if (user.role === 'farmer' && !user.farmer_details) {
          updateData.farmer_details = {
            farm_size: ['5 acres', '10 acres', '15 acres', '20 acres'][Math.floor(Math.random() * 4)],
            farming_experience: ['5 years', '8 years', '12 years', '15 years'][Math.floor(Math.random() * 4)],
            organic_certified: Math.random() > 0.5,
            preferred_crops: ['wheat', 'rice', 'vegetables', 'fruits'].slice(0, Math.floor(Math.random() * 3) + 1)
          };
        }

        if (!user.phone) {
          updateData.phone = `+91${Math.floor(Math.random() * 9000000000) + 1000000000}`;
        }
        ...
        console.error(`Error updating profile for ${user.name}:`, error);
      }
    }
    ...
    console.log('\n✨ Database is now populated with comprehensive sample data!');
    console.log('You can test the application with various user roles and crop data.');
